...
<div class="container">
	<div class="row">
        ...
        @endif
        <form class="form-horizontal" method="POST" action="{{ route('company.address',[Request::segment(3)]) }}">
            {{ csrf_field() }}
            ...
            <div class="form-group{{ $errors->has('country') ? ' has-error' : '' }}">
                ...
                <div class="col-md-4 col-sm-4 col-xs-4 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="country" name="country" style="margin: 0 auto; float: unset;" disabled>
                        ...
                        <?php foreach($countries as $country) : ?>
                            ...
                            <?php else : ?>
                                <option value="<?=$country->id?>" {{(old('country',$company->country) == $country->id) ? 'selected':''}}><?=$country->country_name?></option>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="form-group{{ $errors->has('state_province') ? ' has-error' : '' }}">
                <label for="state_province" class="col-xs-4 control-label text-right text-white">State / Province</label>

                <div class="pull-left col-xs-4 text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="state_province" name="state_province" style="margin: 0 auto; float: unset;" disabled>
                        <option value="">Select</option>
                    </select>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-4 col-xs-4 text-center" style="margin: 0 auto; float: unset;">
                    ...
                    <button type="button" onclick="window.location='<?=route('company.index',[Request::segment(3)])?>'" class="btn yellow-gradient pull-right" style="margin: 0 7px; float: unset;">
                        <strong>Back</strong>
                    </button>
                </div>
            </div>
    ...
    $(function(){
        ...
        let getStateProvince = function(val) {
            $.get( "<?=route('country.state',[''])?>/"+val, function( data ) {
                ...
                for(var a in data) {
                    if('<?=isset($address->state_province_name) ? $address->state_province_name : 'x'?>' == data[a]['id'] || '<?=old('state_province',$company->tax_jurisdiction)?>' == data[a]['id'])
...
<style type="text/css">
    .select2-selection { height: 36px !important; text-align: left; padding: 3px 5px; }
</style>
@stop