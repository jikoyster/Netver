...
<?php foreach($permissions as $permission) : ?>
    <div class="modal fade bs-example-modal-sm<?=$permission->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                'commands': function(column, row)
                {
                    ...
                    varDelete = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"permissions/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
