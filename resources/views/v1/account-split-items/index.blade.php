...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong>Account Split Items</strong></h3>
        </div>
    </div>
    @can('add')
        ...
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-column-id="sub_account_no">Sub-Account No.</th>
                    <th data-column-id="sub_account_name">Sub-Account Name</th>
                    <th data-formatter="commands">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($account_split_items as $item) : ?>
                    <tr>
                        <td><?=$item->id?></td>
                        <td><?=$item->company_chart_of_account->account_no?></td>
                        <td><?=$item->sub_account_no?></td>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body text-center">
        @include('v1.account-split-items.add')
      </div>
    </div>
  </div>
</div>
...
<?php foreach($account_split_items as $item) : ?>
    <div class="modal fade bs-example-modal-sm<?=$item->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            Are you sure you want to delete account split item
            <strong><?=$item->sub_account_name?></strong>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            <a href="<?=route('account-split-item.delete',[$item->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                'commands': function(column, row)
                {
                    ...
                    varDelete = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"account-split-items/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
