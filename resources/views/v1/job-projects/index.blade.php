...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong>Jobs / Projects</strong></h3>
        </div>
    </div>
        ...
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-column-id="jp_id">ID</th>
                    <th data-column-id="name">Name</th>
                    <th data-column-id="location">Company Location</th>
                    <th data-column-id="parent">Parent</th>
                    <th data-column-id="description" data-formatter="descriptions">Description</th>
                    <th data-column-id="index">Index</th>
                    <th data-column-id="has_a_child">Has a child</th>
            ...
            <tbody>
                ...
                function getParent($val) {
                    if($val->jp_parent) {
                        getParent($val->jp_parent);
                        echo $val->jp_parent->jp_id.'.';
                    }
                }
                ?>
                <?php foreach($company->job_projects as $job_project) : ?>
                    <tr>
                        <td><?=$job_project->id?></td>
                        <td>
                            <?php
                                getParent($job_project);
                                echo $job_project->jp_id;
                            ?>
                        </td>
                        <td><?=$job_project->name?></td>
                        <td><?=($job_project->company_location) ? $job_project->company_location->name : ''?></td>
                        <td><?=($job_project->job_project_parent) ? $job_project->job_project_parent->name : ''?></td>
                        <td><?=$job_project->description?></td>
                        <td><?=$job_project->index ? 'Yes':'No'?></td>
                        <td><?=$job_project->has_a_child ? 'Yes':'No'?></td>
                        <td><?=$job_project->active ? 'Yes':'No'?></td>
                        <td>
                            loading...
                        </td>
                    </tr>
...
<?php foreach($company->job_projects as $job_project) : ?>
    <div class="modal fade bs-example-modal-sm<?=$job_project->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            Are you sure you want to delete job / project
            <strong><?=$job_project->name?></strong>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            <a href="<?=route('job-project.delete',[$job_project->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                ...
                'commands': function(column, row)
                {
                    varEdit = '';
                    varDelete = '';
                    varActivity = "<a class=\"btn btn-xs btn-success command-activity\" href=\"job-projects/activity/" + row.id + "\">Activation History</span></a> ";
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"job-projects/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
                    ...
                    @endcan
                    return varActivity+varEdit+varDelete;
                }
            }
