...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong>Company Sales Taxes</strong></h3>
        </div>
    </div>
    @can('add')
        @if(1 == 1)
        <button type="button" onclick="window.location='{{route('company-sales-tax.add')}}'" class="btn pull-right">
            <strong>Import Tax Rates</strong>
        </button>
        @endif
        <button type="button" onclick="window.location='{{route('company-sales-tax.add-grouped')}}'" class="btn pull-right" style="margin-right: 7px;">
        ...
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            ...
            <tbody>
                <?php foreach($tax_rates as $tax_rate) : ?>
                    <tr>
                        <td><?=$tax_rate->id?></td>
                        <td><?=$tax_rate->location ? $tax_rate->company_location->name:''?></td>
                        ...
                        <td><?=$tax_rate->start_date ? $tax_rate->start_date->format('F j, Y') : ''?></td>
                        <td><?=$tax_rate->end_date ? $tax_rate->end_date->format('F j, Y') : ''?></td>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body text-center">
        @include('v1.company-sales-taxes.add')
      </div>
    </div>
  </div>
</div>
      ...
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a href="<?=route('company-sales-tax.delete',[$tax_rate->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                'commands': function(column, row)
                {
                    ...
                    varGrouped = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"company-sales-taxes/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
                    ...
                    if(row.grouped_tax_rates > 0) {
                        varGrouped = " <a class=\"btn btn-xs command-edit\" href=\"company-sales-taxes/edit-grouped/" + row.id + "\"><span class=\"glyphicon glyphicon-compressed\"></span></a> ";
                    }
