	...
	<div class="row" style="margin-bottom: 30px;">
        <form class="form-horizontal" method="POST" action="{{ route('company-sales-tax.save') }}">
            {{ csrf_field() }}

            <div class="form-group">
                <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
                    <h3><strong>Add Company Sales Tax</strong></h3>
                </div>
            </div>
            
            <div class="form-group{{ $errors->has('_location') ? ' has-error' : '' }}">
                <label for="_location" class="col-xs-4 control-label text-right text-white">Location</label>
                ...
                <div class="col-xs-4 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="_location" name="_location">
                        ...
                        @foreach($company_locations as $location)
                            <option value="{{$location->id}}" {{old('_location') == $location->id ? 'selected':''}}>{{$location->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group{{ $errors->has('_tax_agency') ? ' has-error' : '' }}">
                <label for="_tax_agency" class="col-xs-4 control-label text-right text-white">Tax Agency</label>

                <div class="col-xs-4 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="_tax_agency" name="_tax_agency">
                        <option value="">- select -</option>
                        @foreach($company_vendors as $vendor)
                            <option value="{{$vendor->id}}" {{old('_tax_agency') == $vendor->id ? 'selected':''}}>{{$vendor->display_name}}</option>
                        @endforeach
                    </select>
