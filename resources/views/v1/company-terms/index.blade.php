...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong>Company Terms</strong></h3>
        </div>
    </div>
    ...
    @endif
    @can('add')
        <button type="button" class="btn pull-right" onclick="window.location='<?=route('company-term.add')?>'">
            ...
            <tbody>
                <?php foreach($company_terms as $term) : ?>
                    <tr>
                        ...
                        <td><?=$term->data_driven == 1 ? 'before the '.$term->net_due.' th day of the month':'in '.$term->net_due.' day(s)'?></td>
                        <td><?=$term->discount?></td>
                        <td><?=$term->data_driven == 1 ? 'before the '.$term->discount_if_paid.' th day of the month':'in '.$term->discount_if_paid.' day(s)'?></td>
      ...
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a href="<?=route('company-term.delete',[$term->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                'commands': function(column, row)
                {
                    ...
                    varDelete = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"/company-terms/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
