...
<div class="container">
	<div class="row" style="margin-bottom: 30px;">
        <form class="form-horizontal" method="POST" action="{{ route('account-map.edit',[Request::segment(3)]) }}">
            {{ csrf_field() }}

            <div class="form-group">
                <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
                    <h3><strong>Edit Map</strong></h3>
                </div>
            </div>
            ...
            <div class="form-group{{ $errors->has('map_no') ? ' has-error' : '' }}">
                ...
                <div class="col-xs-2 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <input type="text" class="form-control" id="map_no" name="map_no" style="margin: 0 auto; float: unset;" value="{{old('map_no',$account_map->map_no)}}">
                ...
                <div class="col-xs-2 text-center">
                    ...
                    <input type="checkbox" class="form-control pull-right" id="title" name="title" {{old('title',$account_map->title) ? 'checked':''}} style="margin: 7px auto 0; width: 20px; height: 20px;">
                ...
                <div class="col-xs-2 text-center">
                    ...
                    <input type="checkbox" class="form-control pull-right" id="unassignable" name="unassignable" {{old('unassignable',$account_map->unassignable) ? 'checked':''}} style="margin: 7px auto 0; width: 20px; height: 20px;">
            ...
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                ...
                <div class="col-xs-8 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <input type="text" class="form-control" id="name" name="name" style="margin: 0 auto; float: unset;" value="{{old('name',$account_map->name)}}">
                ...
                <div class="col-xs-8 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="parent_id" name="parent_id">
                        ...
                        @foreach($parent_maps as $map)
                            <option value="{{$map->id}}" {{(old('parent_id',$account_map->parent_id) == $map->id) ? 'selected':''}}>{{$map->map_no.' - '.$map->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group{{ $errors->has('account_type_id') ? ' has-error' : '' }}">
                ...
                <div class="col-xs-3 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="account_type_id" name="account_type_id">
                        ...
                        @foreach($account_types as $type)
                            <option value="{{$type->id}}" {{(old('account_type_id',$account_map->account_type_id) == $type->id) ? 'selected':''}}>{{$type->name}}</option>
                        @endforeach
                    </select>
                ...
                <label style="width: 170px;" for="sign_id" class="col-xs-2 control-label text-right text-white">Noram Sign</label>

                <div class="col-xs-3 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="sign_id" name="sign_id">
                        <option value="">- select -</option>
                        @foreach($signs as $sign)
                            <option value="{{$sign->id}}" {{(old('sign_id',$account_map->sign_id) == $sign->id) ? 'selected':''}}>{{$sign->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-xs-8 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="account_group_id" name="account_group_id">
                        ...
                        @foreach($account_groups as $group)
                            <option value="{{$group->id}}" {{(old('account_group_id',$account_map->account_group_id) == $group->id) ? 'selected':''}}>{{$group->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-xs-8 pull-left text-center" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="account_class_id" name="account_class_id">
                        ...
                        @foreach($account_classes as $class)
                            <option value="{{$class->id}}" {{(old('account_class_id',$account_map->account_class_id) == $class->id) ? 'selected':''}}>{{$class->name}}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <div class="col-xs-3 pull-left text-center{{ $errors->has('flip_type') ? ' has-error' : '' }}" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="flip_type" name="flip_type">
                        <option value="">- select -</option>
                        <option value="Individual" {{(old('flip_type',$account_map->flip_type) == 'Individual') ? 'selected':''}}>Individual</option>
                        <option value="Total is Debit" {{(old('flip_type',$account_map->flip_type) == 'Total is Debit') ? 'selected':''}}>Total is Debit</option>
                        <option value="Total is Credit" {{(old('flip_type',$account_map->flip_type) == 'Total is Credit') ? 'selected':''}}>Total is Credit</option>
                    </select>
                ...
                <label style="width: 170px;" for="flip_to" class="col-xs-2 control-label text-right text-white">Flip To</label>
                ...
                <div class="col-xs-3 pull-left text-center{{ $errors->has('flip_to') ? ' has-error' : '' }}" style="margin: 0 auto; float: unset;">
                    <select class="form-control" id="flip_to" name="flip_to">
                        ...
                        @foreach($flip_tos as $map)
                            <option value="{{$map->id}}" {{(old('flip_to',$account_map->flip_to) == $map->id) ? 'selected':''}}>
                                <?php
                                    if($map->parent_map)
            ...
            <div class="form-group{{ $errors->has('has_a_child') ? ' has-error' : '' }}">
                <div class="col-xs-offset-7 col-xs-3 text-center">
                    ...
                    <input type="checkbox" class="form-control pull-right" id="has_a_child" name="has_a_child" {{old('has_a_child',$account_map->has_a_child) ? 'checked':''}} style="margin: 7px auto 0; width: 20px; height: 20px;">
            ...
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-4 col-xs-4 text-center" style="margin: 0 auto; float: unset;">
                    <button type="button" onclick="window.location='{{route('account-maps')}}'" class="btn yellow-gradient" style="margin: 0 7px; float: unset;">
                        <strong>Back</strong>
                    </button>
