	...
	<div class="row" style="margin-bottom: 30px;">
        <form class="form-horizontal" method="POST" action="{{ route('company-account-map.save') }}">
            {{ csrf_field() }}

            <div class="form-group">
                <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
                    <h3><strong>Add Company Account Map</strong></h3>
                </div>
            </div>
            ...
            <div class="form-group{{ $errors->has('nca') ? ' has-error' : '' }}">
                <label for="nca" class="col-xs-2 control-label text-right text-white">NCA<?=($ncas) ? ': '.$ncas->short_name : ''?></label>
                    ...
                    <select type="text" class="form-control" id="nca" name="nca">
                        <option value="">- select -</option>
                            ...
                            @foreach($ncas->lists as $list)
                                <option value="{{$list->id}}" {{old('nca') == $list->id ? 'selected':''}}>{{$list->code.' - '.$list->name}}</option>
                            @endforeach
                            @endif
                    </select>
