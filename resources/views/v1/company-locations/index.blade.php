...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong>Company Locations</strong></h3>
        </div>
    </div>
        ...
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-column-id="cl_id">Location No.</th>
                    <th data-column-id="name">Name</th>
                    <th data-column-id="description" data-formatter="descriptions">Description</th>
                    <th data-column-id="tax_jurisdiction">Tax Jurisdiction</th>
            ...
            <tbody>
                <?php foreach($company->locations as $location) : ?>
                    <tr>
                        <td><?=$location->id?></td>
                        <td><?=$location->cl_id?></td>
                        <td><?=$location->name?></td>
                        <td><?=$location->description?></td>
                        <td><?=$location->tax_jurisdiction ? $location->jurisdiction->state_province_name:''?></td>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body text-center">
        @include('v1.company-locations.add')
      </div>
    </div>
  </div>
</div>
...
<?php foreach($company->locations as $location) : ?>
    <div class="modal fade bs-example-modal-sm<?=$location->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a href="<?=route('company-location.delete',[$location->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                ...
                'commands': function(column, row)
                {
                    ...
                    varDelete = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"company-locations/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
