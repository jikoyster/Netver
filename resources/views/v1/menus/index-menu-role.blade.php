...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong><?=ucfirst($menu->name)?> Roles</strong></h3>
        </div>
    </div>
    <button type="button" onclick="window.location='{{route('menu-elements',[Request::segment(3)])}}'" class="btn yellow-gradient pull-left">
        <strong>Back</strong>
    </button>
    @can('add')
        ...
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-column-id="menu_id" data-visible="false">ID</th>
                    <th data-column-id="name">Name</th>
                    <th data-formatter="commands">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($menu_roles as $menu_role) : ?>
                    <tr>
                        <td><?=$menu_role->id?></td>
                        <td><?=$menu->id?></td>
                        <td><?=$menu_role->name?></td>
                        <td>loading...</td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-body text-center">
            @include('v1.menus.add-menu-role')
          </div>
        </div>
      </div>
    </div>
...
<?php foreach($menu_roles as $menu_role) : ?>
    <div class="modal fade bs-example-modal-sm<?=$menu_role->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            ...
            <strong><?=$menu_role->name?></strong>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            <a href="<?=route('menu.role.delete',[Request::segment(3),$menu->id,$menu_role->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
