...
<div class="col-md-12" style="margin-bottom: 15px;">
    ...
    <div class="form-group">
        <div class="col-md-12 col-sm-12 col-xs-12 text-center text-white" style="margin: 0 auto; float: unset;">
            <h3><strong><?=ucfirst($acl->control_name)?> Permissions</strong></h3>
        </div>
    </div>
    <button type="button" onclick="window.location='{{route('acls')}}'" class="btn yellow-gradient pull-left">
        <strong>Back</strong>
    </button>
    ...
    <div class="col-md-12" style="background-color: #fff; margin-top: 15px;">
        <table id="grid-basic" class="table table-condensed table-hover table-striped">
            ...
            <tbody>
                <?php foreach($aclPermissions as $aclPermission) : ?>
                    <tr>
                        <td><?=$aclPermission->id?></td>
                        <td><?=$aclPermission->name?></td>
                        <td>loading...</td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-body text-center">
            @include('v1.access-control-levels.add-permission')
          </div>
        </div>
      </div>
    </div>
...
<?php foreach($aclPermissions as $aclPermission) : ?>
    <div class="modal fade bs-example-modal-sm<?=$aclPermission->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            Are you sure you want to delete ACL Permission
            <strong><?=$aclPermission->name?></strong>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            <a href="<?=route('acl.permission.delete',[$acl->id,$aclPermission->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
