...
<?php foreach($signs as $sign) : ?>
    <div class="modal fade bs-example-modal-sm<?=$sign->id?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center">
            Are you sure you want to delete sign
            <strong><?=$sign->name?></strong>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
            <a href="<?=route('sign.delete',[$sign->id])?>" class="btn btn-danger">Delete</a>
          </div>
        </div>
      </div>
    </div>
        ...
        $("#grid-basic").bootgrid({
            ...
            formatters: {
                'commands': function(column, row)
                {
                    ...
                    varDelete = '';
                    @can('edit')
                        varEdit = "<a class=\"btn btn-xs btn-primary command-edit\" href=\"signs/edit/" + row.id + "\"><span class=\"glyphicon glyphicon-edit\"></span></a> ";
                    @endcan
                    @can('delete')
